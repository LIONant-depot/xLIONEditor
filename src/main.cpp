...
struct editor
{
    ...
    std::unique_ptr<xeditor::frame>             m_MainFrame;
    ...
    int Init( void )
    {
        ...
        // Create main doc
        //
        m_MainFrame    = std::make_unique<xeditor::frame>(m_MainWindow);
...
int main()
{
    ...
    if (auto Err = Editor->m_MainFrame->CreateTab("AI"); Err) return -1;
    if (auto Err = Editor->m_MainFrame->CreateTab("Project"); Err) return -1;
